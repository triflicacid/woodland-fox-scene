...
export class CupcakesComponent extends DrawComponent {
  _cupcakes = [
    ...
    {x: 345, yFrac: 0.84, scale: 0.9, cupCol: '#ffcc44', pattern: 'plain', accentCol: '#ffaa00', hasCandle: true},
    {x: 470, yFrac: 0.865, scale: 1.0, cupCol: '#88ffcc', pattern: 'dots', accentCol: '#22cc88', hasCandle: true},
   ...
   * @param {object} cup - cupcake
   * @param {number} frame
   * @param {number} index - used to offset animation phase
   */
  _drawCupcake(cup, frame, index) {
    const {ctx} = this;
    ...
    // cup side face (slightly darker, for 3-D feel)
    ...
    this._drawCupPattern(cup, cupW, cupWb, cupH, frame, index);
   ...
   * @param {number} index
   */
  _drawCupPattern(cup, cupW, cupWb, cupH, frame, index) {
    const {ctx} = this;
    ctx.save();
  ...
  /**
   * draw the cream swirl on top of the cup - three stacked dollop circles
   * (matching the cake's _drawCreamDollops style) merged into a cone swirl.
   * @param {number} baseY  - y of the top rim of the cup
   * @param {number} cupW   - half-width of the cup rim
   * @param {number} frame
   * @param {number} index
   */
  _drawCreamSwirl(baseY, cupW, frame, index) {
    const {ctx} = this;
   ...
   * @param {Array<{dx:number, dy:number, rot:number, col:string}>} sprinkles
   * @param {number} baseY
   */
  _drawSprinkles(sprinkles, baseY) {
    const {ctx} = this;
  ...
  /**
   * draw a single candle with black wick and dancing flame - same style as the cake candles.
   * @param {number} tipY  - y position of the top of the cream (candle base sits here)
   * @param {number} frame
   * @param {number} index
   */
  _drawCandle(tipY, frame, index) {
    const {ctx} = this;
    ...
    // pick a colour from DOLLOP_COLS cycling by index
    const candleCol = DOLLOP_COLS[index % DOLLOP_COLS.length];

    // candle body
    ctx.fillStyle = candleCol;
    ctx.beginPath();
    ctx.roundRect(candleX - candleW, candleTopY, candleW * 2, candleH, 1.5);
    ctx.fill();

    // candle highlight
    ctx.fillStyle = 'rgba(255,255,255,0.3)';
    ctx.beginPath();
    ctx.roundRect(candleX - candleW + 1, candleTopY + 1, candleW * 0.5, candleH - 2, 1);
    ctx.fill();

    // wick
    const wickBaseY = candleTopY;
    const wickTipY = candleTopY - 5;
    ctx.strokeStyle = '#2a1808';
    ctx.lineWidth = 1.2;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(candleX, wickBaseY);
