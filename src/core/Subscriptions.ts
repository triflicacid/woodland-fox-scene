...
export class Subscriptions {
     ...
     */
    public static onSceneStateMutation(subscriber: string, onChange: (state: SceneState) => void) {
     ...
     */
    public static captureAll(subscriber: string, onChange: (payload: unknown) => void) {
        return new Subscription(Subscription.CAPTURE_ALL, subscriber, onChange);
    }

    public static onSeasonChange(subscriber: string, onChange: (change: ValueChange<Season>) => void) {
        return new Subscription(Events.EVENT_SEASON_CHANGE, subscriber, onChange);
    }

    public static onWeatherChange(subscriber: string, onChange: (change: ValueChange<string>) => void) {
        return new Subscription(Events.EVENT_WEATHER_CHANGE, subscriber, onChange);
    }

    public static onTODChange(subscriber: string, onChange: (change: ValueChange<TimeOfDay>) => void) {
        return new Subscription(Events.EVENT_TOD_CHANGE, subscriber, onChange);
    }

    public static onMoonPhaseChange(subscriber: string, onChange: (change: ValueChange<number>) => void) {
        return new Subscription(Events.EVENT_MOON_PHASE_CHANGE, subscriber, onChange);
    }

    public static onSpecialEventChange(subscriber: string, onChange: (change: ValueChange<SpecialEvent>) => void) {
        return new Subscription(Events.EVENT_SPECIAL_EVENT, subscriber, onChange);
    }

    public static onFireworkBang(subscriber: string, onChange: (payload: { loud: boolean }) => void) {
        return new Subscription(Events.EVENT_FIREWORK_BANG, subscriber, onChange);
    }

    public static onLightningStrike(subscriber: string, onChange: (payload: { superBolt: boolean }) => void) {
        return new Subscription(Events.EVENT_LIGHTNING_STRIKE, subscriber, onChange);
    }

    public static onCharacterAction(subscriber: string, onChange: (payload: { character: string; action: string }) => void) {
        return new Subscription(Events.EVENT_CHARACTER_ACTION, subscriber, onChange);
    }

    public static onStatusTextChange(subscriber: string, onChange: (payload: { text: string }) => void) {
        return new Subscription(Events.EVENT_STATUS_TEXT, subscriber, onChange);
    }

    public static onMothronDive(subscriber: string, onChange: () => void) {
        return new Subscription(Events.EVENT_MOTHRON_DIVE, subscriber, onChange);
    }

    public static onMainButtonsStateChange(subscriber: string, onChange: (payload: { enabled: boolean }) => void) {
