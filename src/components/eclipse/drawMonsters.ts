...
const FORM_COUNTS: Record<MonsterType, number> = {
 ...
 */
export function randomMonster(): MonsterType {
 ...
 */
export function randomMonsterForm(type: MonsterType) {
 ...
 */
export function drawMonster(ctx: CanvasRenderingContext2D, type: MonsterType, frame: number, silhouette: boolean, form: number) {
    switch (type) {
        case 'frankenstein':
 ...
 */
function c(col: string, silhouette: boolean) {
    return silhouette ? '#2a1040' : col;
}

// Frankenstein - form 0: classic green suit, form 1: shirtless
export function drawFrankenstein(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean, form = 0) {
...
// Swamp Thing
export function drawSwampThing(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
    ...
    // feet - splayed roots
    ctx.lineWidth = 4;
    ([[-12, 14], [10, 14]] as [number, number][]).forEach(([fx, fy]) => {
        ctx.beginPath();
        ctx.moveTo(fx, fy);
    ...
    if (!silhouette) {
        ...
        ([[-14, -20], [10, -30], [-8, -42], [12, -18]] as [number, number][]).forEach(([vx, vy]) => {
            ctx.beginPath();
        ...
        ctx.lineWidth = 2;
        ([[-10, -20], [0, -15], [8, -25]] as [number, number][]).forEach(([vx, vy]) => {
    ...
    // clawed hands
    ctx.lineWidth = 3;
    ([[-34, -10], [32, -12]] as [number, number][]).forEach(([hx, hy]) => {
        ctx.beginPath();
        ctx.moveTo(hx, hy);
    ...
    if (!silhouette) {
        ctx.fillStyle = '#0d2808';
        ([[-8, -72], [4, -74], [-2, -70]] as [number, number][]).forEach(([lx, ly]) => {
            ctx.beginPath();
...
export function drawReaper(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean, form = 0) {
...
export function drawButcher(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean, form = 0) {
...
// Psycho
export function drawPsycho(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
...
export function drawDeadlySphere(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean, form = 0) {
    ...
    if (form === 0) {
        ...
        ([[-r * 0.3, -r * 0.8], [-r * 0.3, r * 0.8], [r * 0.3, -r * 0.8], [r * 0.3, r * 0.8]] as [number, number][]).forEach(([px, py]) => {
    ...
    if (!silhouette) {
        const coreCol: [string, string, string] = form === 0
            ? ['#ff8020', '#ff4000', '#200800']
            : form === 1
                ? ['#80e0ff', '#20a0e0', '#002040']
...
export function drawVampire(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
...
// Zombie with big eye
export function drawEyezor(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
    ...
    if (!silhouette) {
        ...
        ctx.lineWidth = 0.7;
        ([[-8, -56], [6, -54], [-4, -68], [8, -66], [-9, -62]] as [number, number][]).forEach(([vx, vy]) => {
            ctx.beginPath();
            ctx.moveTo(vx, vy);
...
export function drawFritz(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
...
export function drawPossessed(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean, form = 0) {
    ...
    if (form === 0) { // knight armour
        ...
        if (!silhouette) {
            ...
            ctx.stroke();
            // rivets
            ([[-10, -40], [-10, -20], [10, -40], [10, -20]] as [number, number][]).forEach(([rx, ry]) => {
...
export function drawNailhead(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
    ...
    ctx.lineWidth = 2;
    ctx.lineCap = 'round';
    const nails: [number, number, number, number][] = [
        [-7, -74, -9, -86],
        [-2, -74, -2, -88],
        [3, -74, 4, -87],
        [8, -73, 11, -85],
...
export function drawCreature(ctx: CanvasRenderingContext2D, frame: number, silhouette: boolean) {
    const walk = Math.sin(frame * 0.07) * 5;
    ...
    ([[-10, 16], [9, 16]] as [number, number][]).forEach(([fx, fy]) => {
        ctx.beginPath();
    ...
    ctx.lineWidth = 3;
    ([[-28, -22], [28, -22]] as [number, number][]).forEach(([hx, hy]) => {
        const dir = hx < 0 ? -1 : 1;
        ctx.beginPath();
        ctx.moveTo(hx, hy);
