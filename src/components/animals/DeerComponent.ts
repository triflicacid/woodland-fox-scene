...
type DeerPhase = 'off' | 'entering' | 'grazing' | 'leaving' | 'birthday_bob';

const DEER_PHASES: Record<Exclude<DeerPhase, 'off'>, { f: number }> = {
    entering: {f: 150},
    grazing: {f: 300},
    leaving: {f: 180},
...
export class DeerComponent extends DrawComponent {
    ...
    override draw() {
        ...
        const x = this.x;
        const bob = this.phase === 'birthday_bob' ? Math.sin(frame * 0.1) * 4 : 0;
        ...
        ([[12, 29], [18, 29], [-10, 29], [-4, 29]] as [number, number][]).forEach(([hx, hy]) => {
            ctx.beginPath();
