...
const PECK_ZONES: [number, number][] = [
    [170, 0.87],
    [185, 0.88],
    [430, 0.87],
    [450, 0.88],
...
// fixed spawn zones for wandering chicks [x, y_fraction]
...
export class ChicksComponent extends DrawComponent {
    ...
    override initialise() {
        ...
        this.peckers = PECK_ZONES.map(([x, yF]) => ({
            x,
            yF,
    ...
    override draw() {
        ...
        const all = [
            ...this.wanderers.map(c => ({
                ...c,
                peckAmt: 0,
                y: H * c.yF + Math.sin(frame * 0.18 + c.phase) * 1.5,
            })),
            ...this.peckers.map(c => ({
                ...c,
                peckAmt: Math.max(0, Math.sin(c.peckPhase)) * 6,
                y: H * c.yF,
            })),
        ].sort((a, b) => a.y - b.y);

        all.forEach(c => {
            this.drawChick(c.x, c.y, c.facingRight, c.peckAmt);
        });
    }

    private drawChick(x: number, y: number, facingRight: boolean, peckAmt: number) {
        const {ctx} = this;
