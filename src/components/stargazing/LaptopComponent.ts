...
const MODES: ScreenMode[] = ['off', 'starfield', 'app', 'telescope_feed', 'orrery', 'spectrograph', 'light_curve', 'chat', 'research_paper'];
...
export class LaptopComponent extends DrawComponent {
    ...
    private readonly offsetY = 30;
    private readonly scale = 1.1;
    private readonly laptopW = 28;
    private readonly laptopH = 26;
    private readonly laptopKH = 6;
    ...
    containsPoint(cx: number, cy: number) {
        ...
        // above keyboard - check if within the leaning screen parallelogram
        if (ly < -kh) {
            // left edge leans right as y decreases
    ...
    private drawLaptop() {
        ...
        this.drawScreen(lw, sh, bz, tilt, screenY);
     ...
     */
    private drawScreen(lw: number, sh: number, bz: number, tilt: number, screenY: number) {
        const {ctx} = this;

        // content parallelogram corners: bottom-left, bottom-right, top-right, top-left
        ...
        // move origin to top-left corner, apply horizontal shear so content
        // drawn in (0,0)-(cw,ch) maps onto the leaning screen
        ...
        if (this.mode !== 'off') {
            switch (this.mode) {
                case 'starfield':
                    this.drawStarfieldScreen(0, 0, cw, ch);
                    break;
                case 'app':
                    this.drawAppScreen(0, 0, cw, ch);
                    break;
                case 'telescope_feed':
                    this.drawTelescopeFeedScreen(0, 0, cw, ch);
                    break;
                case 'orrery':
                    this.drawOrreryScreen(0, 0, cw, ch);
                    break;
                case 'spectrograph':
                    this.drawSpectrographScreen(0, 0, cw, ch);
                    break;
                case 'light_curve':
                    this.drawLightCurveScreen(0, 0, cw, ch);
                    break;
                case 'chat':
                    this.drawChatScreen(0, 0, cw, ch);
                    break;
                case 'research_paper':
                    this.drawResearchPaperScreen(0, 0, cw, ch);
                    break;
            }
        }
     ...
     */
    private drawStarfieldScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        ...
        ctx.lineWidth = 0.5;
        ([[0, 1], [1, 2], [2, 3], [3, 4], [4, 0], [1, 5]] as [number, number][]).forEach(([a, b]) => {
            ctx.beginPath();
     ...
     */
    private drawAppScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        const cx = ox + sw * 0.5;
        const cy = oy + sh * 0.52;
        const cr = sh * 0.32;
        ...
        ctx.clip();
        const appStars: [number, number, number, string][] = [
        ...
        ctx.lineWidth = 0.5;
        ([[0, 1], [1, 2], [2, 3], [3, 4]] as [number, number][]).forEach(([a, b]) => {
            ctx.beginPath();
     ...
     */
    private drawTelescopeFeedScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        const cx = ox + sw * 0.5;
        const cy = oy + sh * 0.5;
     ...
     */
    private drawOrreryScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        const cx = ox + sw * 0.5;
        const cy = oy + sh * 0.5;
     ...
     */
    private drawSpectrographScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        ...
        const lines = [
            ...
            {t: 0.14, w: 0.6, label: ''},
            {t: 0.38, w: 1.0, label: 'Hb'},
            {t: 0.52, w: 0.7, label: 'Mg'},
            {t: 0.68, w: 1.2, label: 'Ha'},
     ...
     */
    private drawLightCurveScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        const frame = this.scene.frame;
        ...
        // light curve - flat with transit dip, animate a slow scan line
        ...
        ctx.font = '2.5px sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText('Dflux: 1.2%   P: 385.0d   Rp: 1.6 Re', ox + 2, oy + sh - 1.5);
    }

    /**
     * draw an astronomy discord-style chat with slowly scrolling messages.
     */
    private drawChatScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        const frame = this.scene.frame;
        ...
        const MSGS = [
            ...
            {user: 'NightOwl99', col: '#7289da', lines: ['seeing conditions are perfect']},
            {
                user: 'StarryEyes',
                col: '#43b581',
                lines: ['just spotted the Great Red Spot!', 'took me 20 mins to find it']
            },
        ...
        const scrollY = (frame * 0.12) % totalH;
     ...
     */
    private drawResearchPaperScreen(ox: number, oy: number, sw: number, sh: number) {
        const {ctx} = this;
        const frame = this.scene.frame;
        ...
        ctx.font = '2px sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText('The Astrophysical Journal  Vol. 924', ox + 2, oy + 3.4);
        ...
        ctx.fillText('J. Harrison, M. Chen, A. Petrov', ox + sw * 0.5, oy + 16);
        ...
        const rightLines = [
            ...
            'transit depths exceeding 5s detection',
     ...
     */
    private drawScreenGlow(x: number, y: number) {
        const {ctx} = this;
        const nightAlpha = clamp(1 - this.scene.todBlend * 2.5, 0, 1) * 0.3;
